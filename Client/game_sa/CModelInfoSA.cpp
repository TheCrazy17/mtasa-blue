...
void CModelInfoSA::SetColModel(CColModel* pColModel)
{
    ...
    // MakeCustomModel() re-invokes us right after the model streams in (which is exactly what
    // engineRestreamWorld and friends trigger) specifically to re-apply the custom col over whatever
    // the reload reset the interface's pColModel back to, and that re-apply has to actually run.
    // Without this, a freshly streamed model keeps the original disk collision the reload restored,
    // ignoring our custom one, even though the visual model replacement is reapplied correctly.
