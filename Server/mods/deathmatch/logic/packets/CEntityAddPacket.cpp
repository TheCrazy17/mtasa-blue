        ...
        for (; iter != m_Entities.end(); ++iter)
        {
            ...
            switch (ucEntityTypeID)
            {
                ...
                case CElement::VEHICLE:
                {
                    ...
                    // Written last, after every other vehicle field, so a bug in this (still
                    // experimental) block can never desync the fields read before it.
                ...
                case CElement::PED:
                {
                    ...
                    if (animRunning)
                    {
                        ...
                        const uint64_t elapsedMs =
                            nowTick >= static_cast<uint64_t>(animData.startTime) ? (nowTick - static_cast<uint64_t>(animData.startTime)) : 0;
